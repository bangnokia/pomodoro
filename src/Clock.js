  ...
  return (
    <div>
      <div className="clock relative bg-gradient-to-bl from-yellow-500 to-yellow-200 w-64 h-64 mx-auto rounded-full flex items-center cursor-pointer overflow-hidden">
        ...
        <div className="absolute w-full font-mono text-center text-gray-300 font-semibold tracking-widest text-4xl tracking-wide">
        ...
        <div className="play-button absolute rounded-full w-full h-full" onClick={() => setRunning(!running)}>
          <PlayButton className={`absolute top-1/2 left-1/2 -mt-12 -ml-12 h-24 w-24 text-green-500 hover:text-green-600 cursor-pointer ${running ? 'invisible' : 'visible'}`} />
        </div>
      </div>
    </div>
  );
